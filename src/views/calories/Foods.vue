...
const loader = (skip: number, take: number) =>
  foodStore.loadFoodsPaged(skip, take, appStore.search);
...
<template>
  <Card title="Taomlar" subtitle="Kaloriya bazasidagi taomlar">
    <template #actions>
    ...
    <DataTable :key="tableKey" :loader="loader">
      ...
      <ElTableColumn label="Nomi (UZ)" min-width="200">
        <template #default="{ row }">
          <span class="font-semibold" style="color: var(--text)">{{ row.name?.uz ?? "—" }}</span>
        </template>
      </ElTableColumn>
      <ElTableColumn label="Kategoriya" min-width="160">
        <template #default="{ row }">
          <span v-if="row.categoryName?.uz" class="cat-chip">{{ row.categoryName.uz }}</span>
